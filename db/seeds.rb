...
12.times do |index|
  toilet = Toilet.create!(name: "#{Faker::FunnyName.name} #{index}", price: (5..100).to_a.sample, address: "#{Faker::Address.street_address}, #{Faker::Address.city} #{index}", description: "#{Faker::Quote.jack_handey} #{index}", user: [yann, stuart, denis, alisa, doug, celso].sample)
